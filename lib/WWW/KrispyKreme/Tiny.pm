...
sub locations {
    ...
    my $ua = HTTP::Tiny->new(default_headers => {'X-Requested-With' => 'XMLHttpRequest'});
    ...
    my $c = $ua->get($uri->as_string);
    my $tree = HTML::TreeBuilder::XPath->new_from_content($c->{content});
    ...
    $c = $ua->post_form($status_uri->as_string, {locations => $locations});
    return [] unless $c->{success};

    my $json = JSON::XS::decode_json($c->{content});
